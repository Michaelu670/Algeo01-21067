...
public static void regresiLinierBerganda(int n, int m, double[][] mat, Scanner s) {
...
public static void regresiLinierBerganda(Scanner s) {
		...
		regresiLinierBerganda(n, m, mat, s);
}

public static void regresiLinierBerganda(String sourceFile, Scanner s) {
	Matriks inp = Matriks.readFile(sourceFile);
	double[][] mat = new double[inp.getRow()][inp.getCol()];
	for(int i = 0; i < inp.getRow(); i++) {
		for(int j = 0; j < inp.getCol(); j++) {
			mat[i][j] = inp.getMat(i, j);
		}
	}
	regresiLinierBerganda(inp.getCol() - 1, inp.getRow(), mat, s);
}
